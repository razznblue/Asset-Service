...
const listPaths = async (res, folderName) => {
  ...
  if (!fs.existsSync(path.join(__dirname, "..", "..", "public", folderName))) {
    return {
      msg: `${folderName} data not found. Try triggering a download from the home page`,
    };
  }
  ...
  if (directories) {
    ...
    console.log(
      `Could not list files for folder ${folderName} because directories is an unexpected value: ${directories}`
    );
    return false;
  }
};
