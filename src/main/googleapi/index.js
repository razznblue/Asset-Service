...
import {Asset} from "../../config/models/Asset.js";
dotenv.config();
const {google} = pkg;
...
export const uploadFileToDrive = async (fileOptions, category, folder) => {
  ...
  if (listResponse.status !== 200) {
    console.error(
      "Files Not Found Or Error occured. Response status: " +
        listResponse.status
    );
  }
  ...
  switch (createResponse.status) {
    ...
    default:
      console.error(
        `Error uploading [${filename}] to drive. Status: ${createResponse.status}`
      );
  }
};
...
export const getFolderNames = async (category) => {
  ...
  try {
    ...
    console.error(
      `Error fetching folder for category ${category} -`,
      err?.message
    );
  }
...
const createAssetRecord = async (
  ...
  try {
    ...
    if (!exists) {
      ...
      console.log(`Found existing asset with name ${filename}`);
    }
  } catch (err) {
